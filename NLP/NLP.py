...
def inputNLP(input, returningInput=None):
    ...
    if (ticketdict.get("returning") == True) and (
        (ticketdict.get("return_date") == None) or
...
def dateFormat2(input):
    ...
    if (re.search(r'\d{,2}\/\d{,2}', date) is not None):
        ...
        datenew = datetemp.replace(day=int(strarr[0]),
                                   month=int(strarr[1]),
                                   hour=0,
                                   minute=0,
...
print(
    inputNLP(
        "I want a ticket on the 15/01/20 from Norwich to Cambridge arriving at 5:00pm"
    ))
