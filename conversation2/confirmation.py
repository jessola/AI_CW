...
class ConfirmationRules:
    ...
    def sufficient_ticket_information(self, dep_from, dep_to, date, time, ret):
        self.output_statement(
            'BOT:\tSo if I have this right, you want a {0} ticket from {1} to {2} on {3} at {4}.'
            .format('return' if ret else 'single', dep_from, dep_to, date,
                    time))

        self.output_question(
            'BOT:\tIs this the information you want me to find you a ticket with?\n'
        )

    # Await confirmation
    @Rule(~Question('incorrect_info')
          & DepartingFrom(MATCH.dep_from)
          & DepartingTo(MATCH.dep_to)
          & DepartureTime(date=MATCH.date, time=MATCH.time)
          & Returning(MATCH.ret)
          & AS._input << Input())
    def ticket_information_unconfirmed(self, dep_from, dep_to, date, time, ret,
                                       _input):
        self.retract(_input)
